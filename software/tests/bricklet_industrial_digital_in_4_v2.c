...
typedef void (*WiegandDataLowLevel_CallbackFunction)(uint16_t data_length, bool data_data[256], void *user_data);

typedef void (*WiegandDataAvailable_CallbackFunction)(void *user_data);

typedef void (*WiegandErrorCount_CallbackFunction)(uint32_t framing_error_count, uint32_t overflow_error_count, void *user_data);

typedef void (*WiegandData_CallbackFunction)(bool *data, uint16_t data_length, void *user_data);
...
typedef struct {
	PacketHeader header;
	uint8_t reader_enabled;
	uint16_t bit_count;
	uint32_t bit_timeout;
} ATTRIBUTE_PACKED SetWiegandReaderConfig_Request;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED GetWiegandReaderConfig_Request;
...
typedef struct {
	...
	uint16_t bit_count;
	uint32_t bit_timeout;
} ATTRIBUTE_PACKED GetWiegandReaderConfig_Response;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED ReadWiegandDataLowLevel_Request;

typedef struct {
	PacketHeader header;
	uint16_t data_length;
	uint8_t data_data[32];
} ATTRIBUTE_PACKED ReadWiegandDataLowLevel_Response;

typedef struct {
	PacketHeader header;
	uint8_t data_callback_enabled;
	uint8_t data_available_callback_enabled;
	uint8_t error_count_callback_enabled;
} ATTRIBUTE_PACKED SetWiegandCallbackConfig_Request;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED GetWiegandCallbackConfig_Request;
...
typedef struct {
	...
	uint8_t data_available_callback_enabled;
	uint8_t error_count_callback_enabled;
} ATTRIBUTE_PACKED GetWiegandCallbackConfig_Response;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED GetWiegandErrorCount_Request;

typedef struct {
	PacketHeader header;
	uint32_t framing_error_count;
	uint32_t overflow_error_count;
} ATTRIBUTE_PACKED GetWiegandErrorCount_Response;

typedef struct {
	PacketHeader header;
	uint16_t data_length;
	uint8_t data_data[32];
} ATTRIBUTE_PACKED WiegandDataLowLevel_Callback;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED WiegandDataAvailable_Callback;

typedef struct {
	PacketHeader header;
	uint32_t framing_error_count;
	uint32_t overflow_error_count;
} ATTRIBUTE_PACKED WiegandErrorCount_Callback;
...
static void industrial_digital_in_4_v2_callback_wrapper_wiegand_data(DevicePrivate *device_p, uint16_t data_length, bool data_data[256]) {
	WiegandData_CallbackFunction callback_function;
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA];

	callback_function = (WiegandData_CallbackFunction)device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA];

	if (callback_function != NULL) {
		callback_function(data_data, data_length, user_data);
	}
}
...
static void industrial_digital_in_4_v2_callback_wrapper_wiegand_data_low_level(DevicePrivate *device_p, Packet *packet) {
	WiegandDataLowLevel_CallbackFunction callback_function;
	void *user_data;
	WiegandDataLowLevel_Callback *callback;
	int i;
	bool unpacked_data_data[256];

	if (packet->header.length != sizeof(WiegandDataLowLevel_Callback)) {
		return; // silently ignoring callback with wrong length
	}

	callback_function = (WiegandDataLowLevel_CallbackFunction)device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA_LOW_LEVEL];
	user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA_LOW_LEVEL];
	callback = (WiegandDataLowLevel_Callback *)packet;
	(void)callback; // avoid unused variable warning


	callback->data_length = leconvert_uint16_from(callback->data_length);
	for (i = 0; i < 256; i++) unpacked_data_data[i] = (callback->data_data[i / 8] & (1 << (i % 8))) != 0;

	industrial_digital_in_4_v2_callback_wrapper_wiegand_data(device_p, callback->data_length, unpacked_data_data);

	if (callback_function != NULL) {
		callback_function(callback->data_length, unpacked_data_data, user_data);
	}
}

static void industrial_digital_in_4_v2_callback_wrapper_wiegand_data_available(DevicePrivate *device_p, Packet *packet) {
	WiegandDataAvailable_CallbackFunction callback_function;
	void *user_data;
	WiegandDataAvailable_Callback *callback;

	if (packet->header.length != sizeof(WiegandDataAvailable_Callback)) {
		return; // silently ignoring callback with wrong length
	}

	callback_function = (WiegandDataAvailable_CallbackFunction)device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA_AVAILABLE];
	user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA_AVAILABLE];
	callback = (WiegandDataAvailable_Callback *)packet;
	(void)callback; // avoid unused variable warning
...
static void industrial_digital_in_4_v2_callback_wrapper_wiegand_error_count(DevicePrivate *device_p, Packet *packet) {
	WiegandErrorCount_CallbackFunction callback_function;
	void *user_data;
	WiegandErrorCount_Callback *callback;

	if (packet->header.length != sizeof(WiegandErrorCount_Callback)) {
		return; // silently ignoring callback with wrong length
	}

	callback_function = (WiegandErrorCount_CallbackFunction)device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_ERROR_COUNT];
	user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_ERROR_COUNT];
	callback = (WiegandErrorCount_Callback *)packet;
	(void)callback; // avoid unused variable warning
	...
	callback->overflow_error_count = leconvert_uint32_from(callback->overflow_error_count);

	callback_function(callback->framing_error_count, callback->overflow_error_count, user_data);
}
...
void industrial_digital_in_4_v2_create(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(industrial_digital_in_4_v2, uid, ipcon_p, 2, 0, 1, INDUSTRIAL_DIGITAL_IN_4_V2_DEVICE_IDENTIFIER);
	...
	device_p->response_expected[INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_SET_WIEGAND_READER_CONFIG] = DEVICE_RESPONSE_EXPECTED_FALSE;
	device_p->response_expected[INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_GET_WIEGAND_READER_CONFIG] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	device_p->response_expected[INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_READ_WIEGAND_DATA_LOW_LEVEL] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	device_p->response_expected[INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_SET_WIEGAND_CALLBACK_CONFIG] = DEVICE_RESPONSE_EXPECTED_TRUE;
	device_p->response_expected[INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_GET_WIEGAND_CALLBACK_CONFIG] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	device_p->response_expected[INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_GET_WIEGAND_ERROR_COUNT] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	...
	device_p->callback_wrappers[INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA_LOW_LEVEL] = industrial_digital_in_4_v2_callback_wrapper_wiegand_data_low_level;
	device_p->callback_wrappers[INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA_AVAILABLE] = industrial_digital_in_4_v2_callback_wrapper_wiegand_data_available;
	device_p->callback_wrappers[INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_ERROR_COUNT] = industrial_digital_in_4_v2_callback_wrapper_wiegand_error_count;

	device_p->high_level_callbacks[-INDUSTRIAL_DIGITAL_IN_4_V2_CALLBACK_WIEGAND_DATA].exists = true;
...
int industrial_digital_in_4_v2_set_wiegand_reader_config(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, bool reader_enabled, uint16_t bit_count, uint32_t bit_timeout) {
	DevicePrivate *device_p = industrial_digital_in_4_v2->p;
	SetWiegandReaderConfig_Request request;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_SET_WIEGAND_READER_CONFIG, device_p->ipcon_p, device_p);

	if (ret < 0) {
		return ret;
	}

	request.reader_enabled = reader_enabled ? 1 : 0;
	request.bit_count = leconvert_uint16_to(bit_count);
	request.bit_timeout = leconvert_uint32_to(bit_timeout);
...
int industrial_digital_in_4_v2_get_wiegand_reader_config(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, bool *ret_reader_enabled, uint16_t *ret_bit_count, uint32_t *ret_bit_timeout) {
	DevicePrivate *device_p = industrial_digital_in_4_v2->p;
	GetWiegandReaderConfig_Request request;
	GetWiegandReaderConfig_Response response;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_GET_WIEGAND_READER_CONFIG, device_p->ipcon_p, device_p);
	...
	*ret_reader_enabled = response.reader_enabled != 0;
	*ret_bit_count = leconvert_uint16_from(response.bit_count);
	*ret_bit_timeout = leconvert_uint32_from(response.bit_timeout);

	return ret;
}

int industrial_digital_in_4_v2_read_wiegand_data_low_level(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, uint16_t *ret_data_length, bool ret_data_data[256]) {
	DevicePrivate *device_p = industrial_digital_in_4_v2->p;
	ReadWiegandDataLowLevel_Request request;
	ReadWiegandDataLowLevel_Response response;
	int ret;
	int i;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_READ_WIEGAND_DATA_LOW_LEVEL, device_p->ipcon_p, device_p);
	...
	*ret_data_length = leconvert_uint16_from(response.data_length);
	for (i = 0; i < 256; i++) ret_data_data[i] = (response.data_data[i / 8] & (1 << (i % 8))) != 0;

	return ret;
}

int industrial_digital_in_4_v2_set_wiegand_callback_config(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, bool data_callback_enabled, bool data_available_callback_enabled, bool error_count_callback_enabled) {
	DevicePrivate *device_p = industrial_digital_in_4_v2->p;
	SetWiegandCallbackConfig_Request request;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_SET_WIEGAND_CALLBACK_CONFIG, device_p->ipcon_p, device_p);

	if (ret < 0) {
		return ret;
	}

	request.data_callback_enabled = data_callback_enabled ? 1 : 0;
	request.data_available_callback_enabled = data_available_callback_enabled ? 1 : 0;
	request.error_count_callback_enabled = error_count_callback_enabled ? 1 : 0;
...
int industrial_digital_in_4_v2_get_wiegand_callback_config(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, bool *ret_data_callback_enabled, bool *ret_data_available_callback_enabled, bool *ret_error_count_callback_enabled) {
	DevicePrivate *device_p = industrial_digital_in_4_v2->p;
	GetWiegandCallbackConfig_Request request;
	GetWiegandCallbackConfig_Response response;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_GET_WIEGAND_CALLBACK_CONFIG, device_p->ipcon_p, device_p);
	...
	*ret_data_callback_enabled = response.data_callback_enabled != 0;
	*ret_data_available_callback_enabled = response.data_available_callback_enabled != 0;
	*ret_error_count_callback_enabled = response.error_count_callback_enabled != 0;

	return ret;
}

int industrial_digital_in_4_v2_get_wiegand_error_count(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, uint32_t *ret_framing_error_count, uint32_t *ret_overflow_error_count) {
	DevicePrivate *device_p = industrial_digital_in_4_v2->p;
	GetWiegandErrorCount_Request request;
	GetWiegandErrorCount_Response response;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_IN_4_V2_FUNCTION_GET_WIEGAND_ERROR_COUNT, device_p->ipcon_p, device_p);
	...
	*ret_overflow_error_count = leconvert_uint32_from(response.overflow_error_count);
...
int industrial_digital_in_4_v2_read_wiegand_data(IndustrialDigitalIn4V2 *industrial_digital_in_4_v2, bool *ret_data, uint16_t *ret_data_length) {
	int ret = 0;
	uint16_t data_length;
	bool data_data[256];
	...
	*ret_data_length = 0;
	...
	ret = industrial_digital_in_4_v2_read_wiegand_data_low_level(industrial_digital_in_4_v2, &data_length, data_data);

	if (ret < 0) {
		return ret;
	}

	memcpy(ret_data, data_data, sizeof(bool) * data_length);
